...
class PipelineRunner:
    ...
    def run(self, update_fn):
        ...
        for dataset in self.datasets:
            ...
            for sampler in self.sampling_methods:    
                ...
                target_sizes, dataset_samples = sampler.get_samples(dataset)
                self.current_iteration["target_sizes"] = target_sizes
                self.run_subset(dataset_samples)
                
        return self.results
                
    def run_subset(self, dataset_samples):
        for i, df_mask in enumerate(dataset_samples):
            self.current_iteration["sample_mask"] = df_mask.astype(bool)            
            self.row_info["n_samples"] = np.sum(df_mask)
            self.row_info["target_size"] = self.current_iteration["target_sizes"][i]
            
            _, y_features = self.current_iteration["dataset"].mixed_data(df_mask)
            imbalance_data = create_imbalance_report(np.array(y_features))
            self.row_info["base_mean_ir"] = imbalance_data["mean_ir"]
            self.row_info["base_max_ir"] = imbalance_data["max_ir"]
    ...
    def augmentation_iteration(self, dataset, k, step):
        ...
        X_trainf, y_trainf = dataset.mixed_data(mask)
        ...
        if step_label != "base":
            step_label = int(step_label) if step_label % 1 == 0 else step_label
